...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const pc_link_el = document.querySelector('.pc-navbar');
  if (!pc_link_el) return;

  const pc_link = pc_link_el.innerHTML;
  const pc_tab_menu_list = document.querySelector('.tab-container > .tab-sidemenu > .pc-tab-link');
  const pc_tab_link_list = document.querySelector('.tab-container > .tab-link > .navbar-content > .tab-content');
      ...
      if (elem[l].href == pageUrl && elem[l].getAttribute('href') != '') {
        ...
        while (true) {
          ...
          if (cont.classList.contains('tab-pane')) {
            const active_tab = cont.getAttribute('id');
